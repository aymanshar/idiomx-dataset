...
"""
en_modern_01_prepare_enrichment_batch_requests_v1.py

Prepare enhanced batch requests for the modern idioms/slang branch.

This script is safe to run offline.
It does NOT call the API.
It reads the modern pre-LLM dataset and writes JSONL batch requests
for a richer enrichment schema with:
- idiom validation
- English / Arabic / French canonical meanings
- balanced idiomatic and literal examples
- strict anti-repetition constraints
- stronger modern-slang metadata
- richer multilingual explanations
...
# Full-data defaults
DEFAULT_FULL_INPUT_FILE = BASE_DIR / "data" / "processed" / "idiomx_modern_pre_llm_unique_idioms.parquet"
DEFAULT_FULL_OUTPUT_JSONL = BASE_DIR / "data" / "batches" / "idiomx_modern_batch_v1.jsonl"

# Sample-data defaults
DEFAULT_SAMPLE_DIR = BASE_DIR / "data" / "sample"
DEFAULT_SAMPLE_INPUT_FILE = DEFAULT_SAMPLE_DIR / "idiomx_modern_pre_llm_unique_idioms_sample_v1.parquet"
DEFAULT_SAMPLE_OUTPUT_JSONL = DEFAULT_SAMPLE_DIR / "idiomx_modern_batch_sample_v1.jsonl"
...
Your task is to enrich ONE candidate modern idiom/slang expression into a high-quality structured research dataset.

INPUT
- candidate_expression: "{idiom}"
- existing_meaning_en: "{meaning_en}"
- existing_source_example: "{example}"

CORE INTERPRETATION RULE
Treat the existing source example only as weak optional background evidence.
It may be noisy, incomplete, misleading, literal, repetitive, or low-quality.
...
If is_idiom = true:
- Generate the FULL enrichment:
  - exactly 12 main examples
  - exactly 2 adversarial examples
  - enforce full context balance (6 idiomatic + 6 literal)

If is_idiom = false:
- Do NOT generate any main examples
- Do NOT generate any adversarial examples
- Return:
  - "examples": []
  - "adversarial_examples": []
- Still provide:
  - idiom_canonical (best normalization)
  - idiom_validity_label
  - explanation_en / ar / fr
- Do NOT attempt to force idiomatic usage
- Do NOT invent artificial figurative meanings

OBJECTIVES
1. Determine whether the candidate is truly a valid idiom/slang expression.
2. Normalize the best canonical idiom form.
3. Provide the best canonical English meaning.
4. Provide a natural professional Arabic meaning.
5. Provide a natural professional French meaning.
6. Assign strong metadata about validity, register, region, offensiveness, and slang strength.
7. Generate a controlled balanced set of idiomatic and literal examples.
8. Generate examples that are natural, realistic, and meaningfully different from each other.
9. Provide multilingual explanations and paraphrases.

IDIOM-LEVEL OUTPUT
Return:
- is_idiom
- idiom_validity_label:
  valid_idiom / borderline_expression / not_idiom
- idiom_canonical
...
- regionality:
  general_english / american / british / online_global / uncertain
- offensive_flag:
  none / mild / strong
- explanation_en
- explanation_ar
...
- hard_negative_idioms:
  exactly 3 semantically confusable idioms or close figurative expressions, distinct from the target idiom
- meaning_paraphrases_en:
  exactly 3 short English paraphrases
- meaning_paraphrases_ar:
  exactly 3 short Arabic paraphrases
- meaning_paraphrases_fr:
  exactly 3 short French paraphrases
...
This means:
- 1 idiomatic + 1 literal for dialogue
- 1 idiomatic + 1 literal for narrative
- 1 idiomatic + 1 literal for formal
- 1 idiomatic + 1 literal for social_media
- 1 idiomatic + 1 literal for question
- 1 idiomatic + 1 literal for sarcastic

This balance is mandatory.
Do NOT produce mostly dialogue.
Do NOT underuse sarcastic, formal, or question contexts.
...
These adversarial examples must still be natural and realistic.
...
- Two examples are invalid if they describe the same core event with only superficial wording changes.
- Each example must differ in at least two of the following:
  topic, speaker intention, sentence structure, surrounding vocabulary, emotional tone, or discourse context.
- If any two examples feel structurally or semantically too similar, regenerate them.

STRICT SOURCE-EXAMPLE RULES
- Do NOT copy the provided source example.
- Do NOT lightly paraphrase the source example.
- Do NOT reuse its event, sentence frame, or wording.
- If the source example is weak, ignore it completely.
- If the source example is strong, use it only as weak semantic background, not as a template.

STRICT IDIOM-SENTENCE RULES
- Do NOT use the idiom alone as a sentence.
- Do NOT create examples that are just the idiom plus one or two extra words.
- Sentences should usually be between 8 and 22 words.
- The idiomatic examples must be clearly figurative.
- The literal examples must be clearly literal and natural.

STRICT MEANING RULES
- Meanings must describe the actual contextual meaning, not repeat the idiom.
- Explanations must be specific to the example, not generic boilerplate.
...
- exactly 12 main examples and 2 adversarial examples
- exactly 1 idiomatic and 1 literal example for each context type
- no duplicated or near-duplicated examples
- no copied or lightly rewritten source example
- no shallow examples
- idiom validity judgment is strict and honest
...
def get_response_schema():
    """
    Strict JSON schema for modern enrichment v1.
    """
    return {
        "type": "json_schema",
        "name": "idiomx_modern_v1_enrichment",
        "strict": True,
        "schema": {
            "type": "object",
            "properties": {
...
def create_sample_dataset(
    ...
    """
    Create a small sample dataset from the full pre-LLM modern dataset.
    """
...
def prepare_batch_requests(
    ...
    """
    Convert the modern dataset into JSONL batch requests for the LLM API.
    """
    ...
    with open(output_jsonl, "w", encoding="utf-8") as f:
        for idx, row in df.iterrows():
            ...
            custom_id = str(row_id) if pd.notna(row_id) else f"idiomx_modern_v1_{idx}"
    ...
    print(f"Saved modern batch file to: {output_jsonl}")
...
def parse_args():
    """
    Parse command-line arguments for preparing modern enrichment batch requests.
    """
    parser = argparse.ArgumentParser(description="Prepare IdiomX modern enrichment batch requests v1.")
